...
/***** Defines *****/  //here are to change the stack size and the priority if needed
...
#define NORERADIO_ACK_TIMEOUT_TIME_MS (1)//(160)
...
Task_Handle* NodeRadioTask_init(void) {
    ...
    nodeRadioTaskParams.priority = NODERADIO_TASK_PRIORITY;//try to change
...
void nodeRadioTask_setNodeAddr(uint8_t newNodeAddress)
{
    if(nodeAddress != newNodeAddress)
    {
        ...
        dmSensorPacket.header.packetType = RADIO_PACKET_TYPE_DM_SENSOR_PACKET;   // can change the sensor type
...
static void nodeRadioTaskFunction(UArg arg0, UArg arg1)
{
    ...
    /* Initialize previous Tick count used to calculate up time for the TLM beacon */
    ...
    while (1)
    {
        ...
        if (events & RADIO_EVENT_SEND_ADC_DATA)
        {
            ...
            if (currentTicks > prevTicks)
            {
                ...
                dmSensorPacket.time100MiliSec += ((currentTicks - prevTicks) * Clock_tickPeriod) / 0;//100000;
            }
            else
            {
                //calculate time since last reading in 0.1s units
                dmSensorPacket.time100MiliSec += ((prevTicks - currentTicks) * Clock_tickPeriod) / 0;//100000;
            }
            prevTicks = currentTicks;

            dmSensorPacket.batt = AONBatMonBatteryVoltageGet();
            dmSensorPacket.adcValue[0] = adcData[0];//change to array/////////////////////////////////////////////////////////////////////////////////////////////////////
            dmSensorPacket.adcValue[1] = adcData[1];
            dmSensorPacket.adcValue[2] = adcData[2];
            dmSensorPacket.adcValue[3] = adcData[3];
            dmSensorPacket.adcValue[4] = adcData[4];
           dmSensorPacket.adcValue[5] = adcData[5];
            dmSensorPacket.adcValue[6] = adcData[6];
            dmSensorPacket.adcValue[7] = adcData[7];
            dmSensorPacket.adcValue[8] = adcData[8];
...
enum NodeRadioOperationStatus NodeRadioTask_sendAdcData(uint16_t data[])//input is array
{
    ...
    adcData[3] = data[3];
    adcData[4] = data[4];
    adcData[5] = data[5];//change to array ( add or remove arrays to suit your needs)
    adcData[6] = data[6];
    adcData[7] = data[7];
    adcData[8] = data[8];
...
static void sendDmPacket(struct DualModeSensorPacket sensorPacket, uint8_t maxNumberOfRetries, uint32_t ackTimeoutMs)
{
    ...
    currentRadioOperation.easyLinkTxPacket.payload[2] = (dmSensorPacket.adcValue[0] & 0xFF00) >> 8;
    currentRadioOperation.easyLinkTxPacket.payload[3] = (dmSensorPacket.adcValue[0] & 0xFF);/////////////send an array adcValue

    currentRadioOperation.easyLinkTxPacket.payload[4] = (dmSensorPacket.adcValue[1] & 0xFF00) >> 8;//(dmSensorPacket.batt & 0xFF00) >> 8;
    currentRadioOperation.easyLinkTxPacket.payload[5] = (dmSensorPacket.adcValue[1] & 0xFF);//(dmSensorPacket.batt & 0xFF);
    currentRadioOperation.easyLinkTxPacket.payload[6] = (dmSensorPacket.adcValue[2] & 0xFF00) >> 8;//(dmSensorPacket.batt & 0xFF00) >> 8;
    currentRadioOperation.easyLinkTxPacket.payload[7] = (dmSensorPacket.adcValue[2] & 0xFF);//(dmSensorPacket.batt & 0xFF);
    currentRadioOperation.easyLinkTxPacket.payload[8] = (dmSensorPacket.adcValue[3] & 0xFF00) >> 8;//(dmSensorPacket.batt & 0xFF00) >> 8;
    currentRadioOperation.easyLinkTxPacket.payload[9] = (dmSensorPacket.adcValue[3] & 0xFF);//(dmSensorPacket.batt & 0xFF);
    currentRadioOperation.easyLinkTxPacket.payload[10] = (dmSensorPacket.adcValue[4] & 0xFF00) >> 8;//(dmSensorPacket.batt & 0xFF00) >> 8;
    currentRadioOperation.easyLinkTxPacket.payload[11] = (dmSensorPacket.adcValue[4] & 0xFF);//(dmSensorPacket.batt & 0xFF);
    currentRadioOperation.easyLinkTxPacket.payload[12] = (dmSensorPacket.adcValue[5] & 0xFF00) >> 8;//(dmSensorPacket.batt & 0xFF00) >> 8;
       currentRadioOperation.easyLinkTxPacket.payload[13] = (dmSensorPacket.adcValue[5] & 0xFF);//(dmSensorPacket.batt & 0xFF);
       currentRadioOperation.easyLinkTxPacket.payload[14] = (dmSensorPacket.adcValue[6] & 0xFF00) >> 8;//(dmSensorPacket.batt & 0xFF00) >> 8;
          currentRadioOperation.easyLinkTxPacket.payload[15] = (dmSensorPacket.adcValue[6] & 0xFF);//(dmSensorPacket.batt & 0xFF);
          currentRadioOperation.easyLinkTxPacket.payload[16] = (dmSensorPacket.adcValue[7] & 0xFF00) >> 8;//(dmSensorPacket.batt & 0xFF00) >> 8;
             currentRadioOperation.easyLinkTxPacket.payload[17] = (dmSensorPacket.adcValue[7] & 0xFF);//(dmSensorPacket.batt & 0xFF);
             currentRadioOperation.easyLinkTxPacket.payload[18] = (dmSensorPacket.adcValue[8] & 0xFF00) >> 8;//(dmSensorPacket.batt & 0xFF00) >> 8;
             currentRadioOperation.easyLinkTxPacket.payload[19] = (dmSensorPacket.adcValue[8] & 0xFF);
